...
test_type = "network_update"
try:
    os.mkdir(test_type)
except FileExistsError:
    pass
save_file_name = test_type + "/network_test_log.txt"
...
def train_and_test_agent(architecture, learning_rate, update, test_index):
	i = Interface(
	...
	start = time.time()
	i.train_agent(
        save_file="{}/{}_{}_{}".format(test_type, architecture, update, test_index),
		config=[
		"experienced_replay", update, "frame_skip", "log"
	])
...
for update in ["network_update_delay", "target_network_update_delay"]:
    for index in range(repetitions):
        score, time_diff = train_and_test_agent(
    		architecture=architecture, learning_rate=lr, update=update, test_index=index
    	)
        save_file(architecture, lr, update, score, time_diff)
